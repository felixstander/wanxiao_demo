...
def parse_args() -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(description="Call SalesScenarioMockTools through MCP SSE and poll result")
    parser.add_argument("tool_name", help="要调用的 MCP 工具名称，例如 intelligent_judgment")
    parser.add_argument("--customer-name", help="客户姓名")
    parser.add_argument("--agent-name", default="金牌顾问小安", help="代理人名称（用于 agent_ai_business_card_tool）")
    parser.add_argument("--specialty", default="医疗险+重疾险组合规划", help="代理人擅长领域（用于 agent_ai_business_card_tool）")
    parser.add_argument(
        "--base-url",
        default=os.getenv("MCP_BASE_URL", "http://127.0.0.1:8000"),
    ...
    parser.add_argument(
        ...
        type=float,
        default=60.0,
        help="Seconds to wait for tools/call result",
    )
...
def build_tool_arguments(args: argparse.Namespace) -> dict[str, Any]:
    ...
    返回:
        dict[str, Any]: 工具调用参数字典。
    """
    tool_name = args.tool_name

    # 只需要 customer_name 的工具
    if tool_name in [
        ...
        if not args.customer_name:
            raise RuntimeError(f"Tool {tool_name} requires --customer-name")
        return {"customer_name": args.customer_name}

    # agent_ai_business_card_tool
    if tool_name == "agent_ai_business_card_tool":
        return {
...
def run_sales_mcp_call(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        init_payload = {
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": {"name": "sales-skill-script", "version": "1.0.0"},
            },
        }
        ...
        # Tool call
        tool_call_id = 2
        tool_arguments = build_tool_arguments(args)
        tool_payload = {
            "jsonrpc": "2.0",
            "id": tool_call_id,
            "method": "tools/call",
        ...
        tool_response = wait_for_response(message_queue, error_queue, tool_call_id, timeout_seconds=args.poll_timeout)
...
def main() -> None:
    ...
    args = parse_args()
    try:
        result = run_sales_mcp_call(args)
